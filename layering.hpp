...
namespace detail {
    ...
    // 图的分层数据结构，每个节点在一层，ranking为在第几层
    struct hierarchy {
        vertex_map<int> ranking;                        // 点->层级
        vertex_map<int> pos;                            // 点->该层级位置
        std::vector< std::vector<vertex_t> > layers;    // 层级->所拥有的点
        subgraph& g;
    ...
    /**
     * 将提供的层次结构中跨越多个层的所有边分割成每个仅跨越一个层的段。
     * （也就是将层次结构转换为正确的层次结构）
     * @return 需要被分割的边的列表
     */
