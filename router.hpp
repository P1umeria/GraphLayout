...
namespace detail {
    ...
    private:
        ...
        void set_regular_shifts(const hierarchy& h, edge e) {
            ...
            // 寻找可能的交叉点候选者
            // 如果相邻节点是虚拟顶点，我们也希望检查与最近的普通顶点是否有交集
        ...
        void set_shift(const hierarchy& h, edge e, std::optional<vertex_t> up, std::optional<vertex_t> down, vec2 dirs) {
            ...
            if (up) {
                c_up = pos(*up);
                r_up = h.g.is_dummy(*up) ? shifts[*up][dir_idx(dirs)] : attr.node_size;
            }
            ...
            if (down) {
                c_down = pos(*down);
                r_down = h.g.is_dummy(*down) ? shifts[*down][dir_idx(-dirs)] : attr.node_size;
            }
